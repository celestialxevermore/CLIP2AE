...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None):
        ...
        decoded_output = self.get_vaet_output(vaet_video,video_mask,shaped=True,video_frame = video_frame)
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1):
        ...
        #Case 1 : 그냥 Clip4Clip을 하고 싶은 경우 -> ViT를 Call하는 encode_image
        #Case 3 : 그냥 gsvo는 전혀 건들지 않기 -> ViT를 Call하는 encode image, 그리고 decoded를 vaet_video와 mse_loss구하기 
        #할때 바로 아래의 코드 두개를 사용한다. 
        visual_hidden = self.clip.encode_image(video, video_frame=video_frame).float()
        visual_hidden = visual_hidden.view(bs_pair, -1, visual_hidden.size(-1))

        #Case 2 : 그게 아닌 AE를 통과한 encoded를 토대로 만들어진 x'와 비교하고 싶은 경우 -> VAeT를 Call하는 encode_x_image 
        #visual_hidden = self.clip.encode_vaet_x_image(vaet_video, video_frame=video_frame).float()

        visual_hidden = visual_hidden.type(self.dtype)
        #decoded = decoded.type(self.dtype)
        #visual_hidden = visual_hidden.view(bs_pair, -1, visual_hidden.size(-1))
        
        # visual_output이 visual_hidden과 동일함!! 
    ...
    def get_x_output(self,vaet_video,video_mask,shaped=False,video_frame=-1):
        ...
        if len(vaet_video.shape)!=5:
            a = input("get_x_output에서 지금 vaet_video가 5차원이 아님!")
        
        bs_pair = video_mask.size(0)
        x_hidden = self.clip.encode_vaet_x_image(vaet_video, video_frame=video_frame).float()
        x_hidden = x_hidden.view(bs_pair,-1,x_hidden.size(-1))
    ...
    def get_sequence_vaet_output(self, input_ids, token_type_ids, attention_mask, vaet_video, video_mask, shaped=False, video_frame=-1):
        if shaped is False:
            input_ids = input_ids.view(-1, input_ids.shape[-1])
            token_type_ids = token_type_ids.view(-1, token_type_ids.shape[-1])
            attention_mask = attention_mask.view(-1, attention_mask.shape[-1])
        ...
        if len(vaet_video.shape) !=5:
            a = input("get_sequence_vaet_output에서 지금 vaet_video가 5차원이 아님!")

        sequence_output = self.get_sequence_output(input_ids, token_type_ids, attention_mask, shaped=True)
        vaet_output = self.get_x_output(vaet_video, video_mask, shaped=True, video_frame=video_frame)
    ...
    def _loose_similarity(self, sequence_output, visual_output, attention_mask, video_mask, sim_header="clip4clip"):
        ...
        ##### 여기서부터는 공통적인 부분임 seqLSTM, seqTransf 모두 동일하게 진행됨.
        #20220705 수정
        #visual_output = visual_output / visual_output.norm(dim=-1, keepdim=True)
        #20220627
        #visual_output = self._mean_pooling_for_similarity_visual(visual_output, video_mask)
        visual_output = visual_output / visual_output.norm(dim=-1, keepdim=True)
        #### 20220706 원래는 mean_pooling을 하는게 맞음. 
        # Case 1 : 원래는 mean_pooling을 하는게 맞음. 정규화 -> mean_pooling -> 정규화
        # CLIP4Clip일 때, 그리고 Case 3일 때에만 아래 두 코드를 실행해야함.
        visual_output = self._mean_pooling_for_similarity_visual(visual_output, video_mask)
        visual_output = visual_output / visual_output.norm(dim=-1, keepdim=True)
    ...
    def _vaet_similarity(self, sequence_output, vaet_output, attention_mask, video_mask, sim_header="meanP"):
        ...
        if self.training:
            vaet_output = allgather(vaet_output, self.task_config)
            video_mask = allgather(video_mask, self.task_config)
            sequence_output = allgather(sequence_output, self.task_config)
            torch.distributed.barrier()
        
        if len(vaet_output.shape)!=3:
            a = input("_vaet_similarity에 있는 vaet_output이 3차원이 아님!")


        vaet_output = vaet_output / vaet_output.norm(dim=-1, keepdim=True)
        vaet_output = self._mean_pooling_for_similarity_visual(vaet_output, video_mask)
        vaet_output = vaet_output / vaet_output.norm(dim=-1, keepdim=True)
