...
class AUTOENCODER(nn.Module):
    def __init__(self, channel, width : int, height : int, linear_patch='3d'):
        ...
        def EncoderLayer(in_channel,out_channel,pooling_size=None):
            network=[] 
            # self.conv2 = nn.Conv3d(in_channels=128,out_channels=width,kernel_size=(4,4,1),
        ...
        self.encoder1 = EncoderLayer(channel, 3, pooling_size=(1,1,2))
        
        self.encoder2 = EncoderLayer(3, 3, pooling_size=(1,1,2))
        self.encoder3 = EncoderLayer(3, 3, pooling_size=(1,1,2))
        self.encoder4 = EncoderLayer(3, 3, pooling_size=(1,1,2))

        self.decoder1 = DecoderLayer(3, 3, size=(1,1,2))
        self.decoder2 = DecoderLayer(3, 3, size=(1,1,2))
        self.decoder3 = DecoderLayer(3, 3, size=(1,1,2))
        self.decoder4 = DecoderLayer(3, channel, size=(1,1,2))
    ...
    def forward(self, x ):
        ...
        #decoded = torch.cuda.HalfTensor(decoded)
...
class VAeT(nn.Module):
    def __init__(self,input_resolution : int, patch_size : int, width : int, layers : int, heads : int, output_dim : int, linear_patch :str = '2d'):
        ...
        self.AE = AUTOENCODER(channel=3,width=input_resolution,height=input_resolution)
        ...
        if self.linear_patch == '3d':
            self.conv2 = nn.Conv3d(in_channels=3,out_channels=width,kernel_size=(1,patch_size,patch_size),
                stride = (1,patch_size,patch_size),padding=(0,0,0),bias=False)
